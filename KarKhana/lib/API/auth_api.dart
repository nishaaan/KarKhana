...
//Edit profile name
Future<dynamic> editProfile(String key, String name) async {
  Map body = {"full_name": name};
  var url = Uri.parse("$baseUrl/accounts/auth/user/");
  var res = await http.put(url,
      headers: {'Authorization': 'Token ${key}'}, body: body);
  print(res.body);
...
class FetchCategoryDeals {
  var data = [];
  List<Deals> results = [];
  String api = '$baseUrl/accounts/deals/';
  Future<List<Deals>> getCategoryDeals({String? query}) async {
    var url = Uri.parse(api);
    var res = await http.get(url);
    try {
      if (res.statusCode == 200) {
        ...
        if (query != null) {
          results = results
              .where((element) => element.Deals_category!
                  .trim()
                  .toLowerCase()
                  .contains(query.trim().toLowerCase()))
              .toList();
...
// Post Review posting API
Future<dynamic> postReviewRating(
  String Rating,
  String Review,
  String BusinessName,
  String CustomerName,
  ...
  String pk = BusinessID.toString();
  var url = Uri.parse("$baseUrl/accounts/ReviewRating/");
  var res = await http.post(url,
      headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
      },
...
// Get Review API
Future<List<ReviewsRatings>> GetReviewsRating(int id) async {
  List<ReviewsRatings> myTodos = [];
  String pk = id.toString();
  const String api = '$baseUrl/accounts/ReviewRating/';
  var res = await http.get(Uri.parse(api + pk));
  print(res.statusCode);
  print(res.body);
  ...
  if (res.statusCode == 200 || res.statusCode == 201) {
    var data = jsonDecode(res.body);
    data.forEach((todo) {
      ReviewsRatings t = ReviewsRatings(
        ReviewRating_ID: todo["id"],
        Business_Name: todo["Business_Name"],
