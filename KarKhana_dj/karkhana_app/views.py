...
from .models import Cart, Charges, CouponActive, Deals, EmailNotificationModel, ReviewsRatings, User, VendorBusinessDetails, VendorRequest, Feedback
...
class ReviewRatingViews(generics.ListCreateAPIView):
    queryset = ReviewsRatings.objects.all()
    serializer_class = ReviewsRatingsSerializer

class ReviewsRatingsByBusiness(generics.ListCreateAPIView):
    serializer_class = ReviewsRatingsSerializer
    
    def get_queryset(self):
        business = self.kwargs['business']
        queryset = ReviewsRatings.objects.filter(Business_ID = business)
        return queryset